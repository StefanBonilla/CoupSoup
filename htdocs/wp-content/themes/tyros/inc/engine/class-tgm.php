...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            ...
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'tyros' ),
                'menu_title'                     => __( 'Install Plugins', 'tyros' ),
                'installing'                     => __( 'Installing Plugin: %s', 'tyros' ),
                'oops'                           => __( 'Something went wrong.', 'tyros' ),
                ...
                'return'                         => __( 'Return to Required Plugins Installer', 'tyros' ),
                'dashboard'                      => __( 'Return to the dashboard', 'tyros' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'tyros' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'tyros' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'tyros' ),
                'dismiss'                        => __( 'Dismiss this notice', 'tyros' ),
            );
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'tyros' ) . '">' . __( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ) . '</p>';
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'tyros' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'tyros' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'tyros' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'tyros' ) : __( 'Recommended', 'tyros' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'tyros' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'tyros' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'tyros' ) . ' %2$s">' . __( 'Install', 'tyros' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'tyros' ) . ' %2$s">' . __( 'Activate', 'tyros' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'tyros' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'tyros' ),
                'source' => __( 'Source', 'tyros' ),
                'type'   => __( 'Type', 'tyros' ),
                'status' => __( 'Status', 'tyros' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tyros-bulk-install'  => __( 'Install', 'tyros' ),
                'tyros-bulk-activate' => __( 'Activate', 'tyros' ),
            );
